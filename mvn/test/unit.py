...
class productTester(myTests):
    ...
    def testMvnMul(self):
        self.assertTrue( (fix.A*fix.B).cov == (fix.A*fix.B.transform()+fix.B*fix.A.transform()).cov/2 )
        if not (fix.A.flat or fix.B.flat): 
            self.assertTrue( (fix.A*fix.B).mean == (fix.A*fix.B.transform()+fix.B*fix.A.transform()).mean/2 )
...
class linalgTester(myTests):
    ...
    def testDist2(self):
        if not fix.A.flat:
            self.assertTrue( Matrix((fix.A**0).dist2(numpy.zeros((1,fix.ndim))))==helpers.mag2((fix.A**0).mean) )
